package com.epam.hotel.web.util;

public class URLConstants {

    public final static String ROOM_LIST_PAGE = "/WEB-INF/jsp/RoomList.jsp";
    public final static String ROOM_INFO_PAGE = "/WEB-INF/jsp/RoomInfo.jsp";
    public final static String REGISTER_PAGE = "/WEB-INF/jsp/Registration.jsp";
    public final static String RETRY_REGISTRATION_COMMAND = "ControllerServlet?command=register_page&";
    public final static String GO_TO_INDEX = "index.jsp";
    public final static String INDEX_PAGE = "/WEB-INF/jsp/IndexPage.jsp";
    public final static String LOGIN_PAGE = "/WEB-INF/jsp/SignIn.jsp";
    public final static String LOGIN_PAGE_COMMAND = "ControllerServlet?command=login_page";
    public final static String ORDER_DETAILS_COMMAND = "ControllerServlet?command=order_details";
    public final static String ORDER_DETAILS_PAGE = "/WEB-INF/jsp/OrderDetails.jsp";
    public final static String CONTROL_PAGE = "/WEB-INF/jsp/Control.jsp";
    public final static String PROFILE_PAGE = "/WEB-INF/jsp/Profile.jsp";
    public final static String SEARCH_RESULT_PAGE = "/WEB-INF/jsp/SearchResult.jsp";
    public final static String CHANGE_ORDER_CONGRATS_PAGE_COMMAND = "ControllerServlet?command=change_order_congrats_page";

}
